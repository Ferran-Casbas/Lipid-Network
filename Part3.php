...
/* Section that executes query and displays the results */  
	...
	$result = mysqli_query($cxn,$sql);
 	if ($_POST['Scored']=="Yes"){
		$sql="select Substrate,TypeNode,COALESCE(ScoreU,0) from tem left join ScoreNodes on Substrate = Node";
	...
	function IsInTheList($value,$l) {   #THE FUCTION CHECKS IF A VALUE IS IN THE LIST L 
		...
		foreach ($l as $i){
			if($value==$i[0]){
				array_push($templ,$i[1]);
				}
			}
		if (count($templ) ==0){
			return False;
		} elseif (count($templ) == 1) {
			return $templ[0];
		} else {
			#print_r($templ);
			return $templ;
	...
	########First GO
	##Now we go throug the Network finding the Nodes that are excited then afecting the score of the one they excite.
	$lexited = array();									#Contains info of who is exited and who exite it
	foreach ($Network as $i) {
		$i = str_replace("\n", "",$i);
		$l = explode("\t",$i);
		if ($NodeInfo[$l[0]][1]!= 0){
			$NodeInfo[$l[2]][2]-=$NodeInfo[$l[0]][1];
			$temp = array($l[2],$l[0]);
			array_push($lexited,$temp);
			}
		if ($NodeInfo[$l[2]][1]!= 0){
			$NodeInfo[$l[0]][2]+=$NodeInfo[$l[2]][1];
			$temp = array($l[0],$l[2]);
			array_push($lexited,$temp);
			}	
	}
	$lexited = array_unique($lexited, SORT_REGULAR);
	fclose($file);

	############Second GO ######################
	$WhoDiDExciteMe = array();
	foreach ($Network as $i) {
		$once=False;
		$i = str_replace("\n", "",$i);
		$l = explode("\t",$i);	
		$From=IsInTheList($l[0],$lexited);
		if($From and $NodeInfo[$l[2]][1]==0.0){					#and $NodeInfo[$l[2]][1]==0.0   IsNew($l[0],$lexited,$l[2])
			$WasExcited= False;
			$lInitials = $WhoDiDExciteMe[$l[2]];
			if ($lInitials!= "") {
				foreach ($lInitials as $a){
					if ($a == $From){
						$WasExcited = True;
						break;
					}
				}
				array_push($WhoDiDExciteMe[$l[2]],$From);
			} else {
				$WhoDiDExciteMe[$l[2]]=$From;
			}
			if ($WasExcited==False){
				$NodeInfo[$l[2]][2]-=$NodeInfo[$l[0]][2];
			}
			$once=True;
		}
		$From=IsInTheList($l[2],$lexited);
		if($From and $once == False and $NodeInfo[$l[0]][1]==0.0){			#and $NodeInfo[$l[0]][1]==0.0 
			$WasExcited= False;
			$lInitials = $WhoDiDExciteMe[$l[0]];
			if ($lInitials!= "") {
				foreach ($lInitials as $a){
					if ($a == $From){
						$WasExcited= True;
				...
				array_push($WhoDiDExciteMe[$l[0]],$From);
			} else {
				$WhoDiDExciteMe[$l[0]]=$From;
			}
			if ($WasExcited==False){
				$NodeInfo[$l[0]][2]+=$NodeInfo[$l[2]][2];
			}
			$once= True;
		}
	}
	...
	$zip->addFile("Gloval-Metabolomics-Layout.cys");
